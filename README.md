...
| [security-vulnerability-review](skills/security-vulnerability-review/SKILL.md) | Produces a structured Application Security Protection mitigation document for a specific finding — maps the data flow to concrete code-level controls, verdicts false positives, and outputs a wiki-ready section (covers XSS, SQL Injection, Path Traversal, Client-Side Injection, Auth) | A SAST tool (Checkmarx, SonarQube) flags a vulnerability and you need code-level evidence of mitigation, a justified false-positive, or a minimal fix |
...
│   ├── security-vulnerability-review/ #   Review
